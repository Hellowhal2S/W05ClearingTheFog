...
void AEditorPlayer::Input()
{
    ...
    if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
    {
        if (!bLeftMouseDown)
        {
            ...
            uint32 UUID = GetEngine()->RenderEngine.GetGraphicsDevice()->GetPixelUUID(mousePos);
...
{
	// ���� ��ȯ ���
    FMatrix worldMatrix = obj->GetComponentTransform();
...
void AEditorPlayer::ControlRotation(USceneComponent* pObj, UGizmoBaseComponent* Gizmo, int32 deltaX, int32 deltaY)
{
    ...
    FQuat currentRotation = pObj->GetRelativeQuat();
